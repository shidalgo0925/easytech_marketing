#!/usr/bin/env python3
"""Tests M11.3 — Memory events + roadmap summary after enrich."""
...
class MarketingBrainM113Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = tempfile.mkdtemp(prefix="mb_m113_")
    ...
    def test_roadmap_enrich_writes_summary_and_memory(self) -> None:
        ...
        self.assertIn("roadmap_summary", body)
        self.assertIn("ai_enrichment", body["roadmap_summary"])
        self.assertEqual(body["roadmap_summary"]["ai_enrichment"]["enriched_count"], rec_count)

        conn = get_connection()
        try:
            summary_row = conn.execute(
                "SELECT summary_json FROM daily_roadmaps WHERE tenant_id = ?",
                (self.tenant_id,),
            ).fetchone()
            roadmap_event = conn.execute(
                """
                SELECT event_type FROM memory_events
                WHERE tenant_id = ? AND event_type = 'DailyRoadmapEnriched'
                """,
                (self.tenant_id,),
            ).fetchone()
            rec_events = conn.execute(
                """
                SELECT COUNT(*) FROM memory_events
                WHERE tenant_id = ? AND event_type = 'RecommendationEnriched'
                """,
                (self.tenant_id,),
            ).fetchone()
        finally:
        ...
        self.assertIn("ai_enrichment", summary)
        self.assertIsNotNone(roadmap_event)
        self.assertEqual(rec_events[0], rec_count)

    def test_single_enrich_memory_event(self) -> None:
        gen = self.client.post(
            f"/api/v1/tenants/{self.tenant_id}/roadmaps/{self.roadmap_date}/generate"
        )
        rec_id = gen.get_json()["data"]["recommendations"][0]["recommendation_id"]

        enrich = self.client.post(
            f"/api/v1/tenants/{self.tenant_id}/recommendations/{rec_id}/enrich",
            json={"persist": True},
        )
        ...
        self.assertEqual(row[0], "RecommendationEnriched")
